...
if (!gotTheLock) {
  ...
  app.on("second-instance", (event, commandLine, workingDirectory) => {
    ...
    if (mainWindow) {
      ...
      // Check for .mn file in commandLine
      const filePath = commandLine.find((arg) => arg.endsWith(".mn"));
      if (filePath) {
        // Handle file open
        openFile(filePath).then((data) => {
          if (data) {
            mainWindow?.webContents.send(IPC_EVENTS.FILE_OPENED, data);
          }
  ...
  app.whenReady().then(() => {
    ...
    const filePath = process.argv.find((arg) => arg.endsWith(".mn"));
    if (filePath) {
      // Wait for window to be ready? Or just try opening after creation.
      // It's better to wait for the renderer to be ready.
      mainWindow?.webContents.once('did-finish-load', () => {
        openFile(filePath).then((data) => {
          if (data) {
            mainWindow?.webContents.send(IPC_EVENTS.FILE_OPENED, data);
          }
        }).catch(err => console.error("Error opening file on startup:", err));
      });
    }
  ...
  async function openFile(filePath: string) {
    try {
      ...
      const { tempDirPath, mindmapData, markdownFiles } =
        await fileOperations.unpackMnFile(filePath);

      // Track the new temp directory
      activeTempDir = tempDirPath;

      return { filePath, tempDirPath, mindmapData, markdownFiles };
    } catch (error) {
  ...
  ipcMain.handle(IPC_EVENTS.FILE_OPEN, async (event) => {
    try {
      ...
      return await openFile(finalFilePath);
    } catch (error) {
      console.error("Main process: Failed to open .mn file via dialog", error);
      throw error;
    }
  });
